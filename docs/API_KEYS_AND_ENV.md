...
| `GEMINI_API_KEY` | **ai-gateway** | Optional | Google Gemini API key. **Primary** when set (including when both keys exist—matches free-tier usage in AI Studio). Same tasks with `GEMINI_MODEL` (default `gemini-2.0-flash`). |
