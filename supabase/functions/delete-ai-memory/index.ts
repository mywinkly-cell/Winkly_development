...
serve(async (req) => {
  ...
  const userClient = createClient(supabaseUrl, supabaseAnonKey, {
    global: { headers: { Authorization: authHeader, ...Object.fromEntries(cors) } },
  });
  ...
  try {
    ...
    if (mode === "all") {
      ...
      return withCorsJson(req, { error: "Invalid mode" }, { status: 400 });
    }
    ...
    return withCorsJson(req, { error: "Delete AI memory failed" }, { status: 500 });
  }
});
